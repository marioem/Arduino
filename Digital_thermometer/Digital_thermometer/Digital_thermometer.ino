...
int (*displayBuf)[8];      // Probably can be removed. displayBuf in now local variable to LC4LEDDisplay
float temperature;
char *temperatureStr;
int readDelay = 1000;
...
void loop() {
  ...
  if(readDelay == 0)
    readDelay = 1000;
  LC4LEDDisplay(temperatureStr);
}


/************************************************************************
 * LC4LEDDisplay - display string on a 4 LED 7 segmend LED display
 * module from LC Technology.
 * 
 * Input:
 * - digits - pointer to a char array of "digits" to display
 * 
 * Output:
 * - nothing
 * 
...
void LC4LEDDisplay(char *digits)
{
  int bufIndex = 0, len = 0, dots = 0, dotsPos[4] = {
    0,0,0,0                    }
  , j = 0;
  char displayBuf[4], currentChar;
  boolean error = FALSE;
